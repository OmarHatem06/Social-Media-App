...
class UserService {
    ...
    acceptFriendRequest = async (req, res) => {
        ...
        if (!request)
            throw new BadRequestException("cannot find his request");
        await UserModel.updateOne({ _id: req.user._id }, {
            $push: { friends: request.SendFrom },
        });
        await UserModel.updateOne({ _id: request.SendFrom }, {
    ...
    rejectFriendRequest = async (req, res) => {
        ...
        if (!request)
            throw new BadRequestException("cannot found the request");
        return res
            .status(200)
    ...
    blockUser = async (req, res) => {
        ...
        const alreadyblocked = me.blockedUsers?.some((id) => id.equals(TargetUser._id));
        if (alreadyblocked)
            throw new BadRequestException("user already is blocked");
        if (me.blockedBy?.some((id) => id.equals(TargetUser._id))) {
            throw new BadRequestException("he blocked you first u cannot block him back");
        }
        await Promise.all([
            ...
            UserModel.updateOne({ _id: TargetUser._id }, {
